  ...
  return (
    <section className="py-24 px-4 max-w-7xl mx-auto">
      <div className="text-center mb-16">
        <h2 className="text-4xl md:text-5xl font-display font-bold mb-4 text-dark">{benefits.title}</h2>
        <p className="text-slate-500 max-w-2xl mx-auto">Oportunidades únicas em um mercado com crescimento acelerado e segurança jurídica.</p>
      </div>
        ...
        {benefits.cards.map((card, index) => {
          ...
          return (
            <motion.div
              ...
              className={`${isWide ? 'md:col-span-2' : ''} bg-dark hover:bg-darker transition-colors duration-300 rounded-3xl p-8 border border-white/10 shadow-lg relative overflow-hidden group`}
            >
              <motion.div 
              ...
              <h3 className="text-2xl font-display font-bold mb-3 relative z-10 text-white">{card.title}</h3>
              <p className="text-white/80 relative z-10">{card.description}</p>
            </motion.div>
          );
        })}
      </div>
